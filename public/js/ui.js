...
window.WeVidHere.ui = (window.WeVidHere.ui || (function () {
	...
	var dom = (function (){
		...
		* returning an exisiting DOM element
		* @return {DOMObject} the DOM object, with the id if elem = a string 
		*/
		function getElement(elem){
			...
			if(typeof elem == "string") {
				...
				if(!elem){
					console.log("ivalid id string passed to getElement:" + elem);///////////////////////////
					return null;
				}
			} else {
				console.log("elem not type string,if it is a:" + elem);//////////////////////////////
				el = elem;
			}
			return elem;
		}
		...
		/*
		 * @method bind
		 * add and event to a DOM element
	...
	* - change ARIA visibility to visible
	* - swap visibility class 
	* @param {String} sceenId the id="xxx" of the HTML tag
	*/
	function showScreen(screenId){
		console.log("im in showScreen .ui my id is :" + screenId);
		...
		for(var i = 0; i < myScreens.length; i++){
			...
			//console.log("current id" + ce[i].id);
			var ce = myScreens[i];
			if (ce.id === screenId) {
				//make visible
